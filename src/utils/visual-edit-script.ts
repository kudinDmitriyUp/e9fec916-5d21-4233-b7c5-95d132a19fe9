...
(function() {
  ...
  let paragraphEditRoot = null;
  let originalParagraphContent = null;
  let isEditing = false;
  ...
  const getParagraphRoot = (el) => {
    if (!el || typeof el.closest !== 'function') return el;
    const block = el.closest(
      'p, li, h1, h2, h3, h4, h5, h6, blockquote, td, th, figcaption, [role="paragraph"]',
    );
    return block || el;
  };
  ...
  const makeEditable = (element, clickEvent) => {
    ...
    paragraphEditRoot = getParagraphRoot(element);
    originalParagraphContent = paragraphEditRoot ? paragraphEditRoot.textContent : element.textContent;
    ...
    const handleInput = () => {
      ...
      const paragraphNew = paragraphEditRoot ? paragraphEditRoot.textContent : currentText;
      ...
      window.parent.postMessage({
        ...
        data: {
          ...
          oldValue: originalParagraphContent,
          newValue: paragraphNew,
      ...
      if (currentText !== originalParagraphContent) {
        window.parent.postMessage({
  ...
  const makeUneditable = (element, save = false) => {
   ...
   const paragraphChanged = paragraphEditRoot
      ? originalParagraphContent !== (paragraphEditRoot.textContent || '')
      : originalContent !== element.textContent;

    if (save && paragraphChanged) {
      const elementInfo = getElementInfo(element);
      ...
      let finalParagraphNew = paragraphEditRoot ? paragraphEditRoot.textContent : finalText;
      if (finalParagraphNew === ' ' || (typeof finalParagraphNew === 'string' && finalParagraphNew.trim() === '')) {
        finalParagraphNew = '';
      }
    ...
    paragraphEditRoot = null;
    originalParagraphContent = null;
  };
